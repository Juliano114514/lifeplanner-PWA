...
export function WishesPage({ account, sync, onEditing }: { account: Account; sync: () => void; onEditing: (value: boolean) => void }) {
  const { kind: routeKind } = useParams();
  const kind = kinds.find(value => value.toLowerCase() === routeKind);
  ...
  const wishes = materializePlanner(account).wishes.filter(wish => wish.kind === kind && wish.isArchived === archived &&
  ...
  function card(wish: Wish) {
    ...
    return <article className={`task-card planned-task-card wish-card ${done ? 'is-done' : ''}`} key={wish.id}>
      <div className="task-overview"><span className="avatar task-avatar" aria-hidden="true">{avatar(wish.ownerId) ? <img src={avatar(wish.ownerId)} alt="" /> : name(wish.ownerId).slice(0, 1)}</span>
        ...
        <input className="task-checkbox" type="checkbox" checked={done} disabled={archived || blocked} aria-label={`${done ? '恢复' : '完成'}愿望：${wish.name}`} onChange={() => act({ type: 'wishStatus', id: wish.id, status: done ? 'PENDING' : 'COMPLETED' })} />
        ...
        <details className="task-details"><summary>操作与记录</summary>{!archived && <div className="actions">
          ...
          <button disabled={blocked} onClick={() => act({ type: 'wishPin', id: wish.id, pinned: !wish.isPinned })}>{wish.isPinned ? '取消置顶' : '置顶'}</button>
          <button disabled={blocked} onClick={() => act({ type: 'wishStatus', id: wish.id, status: done ? 'PENDING' : 'COMPLETED' })}>{done ? '恢复待办' : '完成'}</button>
          <button disabled={blocked} onClick={() => { if (window.confirm(`归档「${wish.name}」？归档后将从清单隐藏。`)) act({ type: 'archiveWish', id: wish.id }); }}>归档</button></div>}
  ...
  return <><PageHeading title={wishKinds[kind]} action={<button className="primary" disabled={blocked} onClick={() => setEditor('new')}>新增愿望<span>＋</span></button>} />
    <Link className="text-button back-link" to="/wishes">← 返回愿望</Link>
    <div className="filter-row"><div className="segmented" aria-label="愿望归属筛选">{[['all', '全部'], ['mine', '我的'], ['other', '对方的']].map(([value, label]) => <button key={value} aria-pressed={filter === value} className={filter === value ? 'selected' : ''} onClick={() => setFilter(value)}>{label}</button>)}</div><button className="text-button" aria-pressed={archived} onClick={() => setArchived(!archived)}>{archived ? '返回清单' : '查看归档'}</button></div>
    {error && <p className="notice error-text" role="alert">{error}</p>}
    {account.plannerPending.some(value => ['saveWish', 'wishPin', 'wishStatus', 'archiveWish'].includes(value.command.operation.type)) && <p className="hint">愿望有待同步的修改</p>}
    {archived ? <section className="task-section"><SectionHeading title="已归档" count={wishes.length} />{wishes.sort((a, b) => b.updatedAt - a.updatedAt).map(card)}{!wishes.length && <p className="empty">还没有归档的愿望。</p>}</section> : <>
      <section className="task-section"><SectionHeading title="想要实现" count={pending.length} />{pending.map(card)}{!pending.length && <p className="empty">把想去、想吃、想买的事记下来，慢慢实现。</p>}</section>
      <section className="task-section"><SectionHeading title="已经实现" count={completed.length} />{completed.map(card)}{!completed.length && <p className="empty">实现的愿望会留在这里。</p>}</section></>}
    {editor && <WishEditor key={editor === 'new' ? 'new' : editor.id} initial={editor === 'new' ? undefined : editor} kind={kind} account={account} onClose={() => setEditor(null)} onSave={async draft => { await run({ type: 'saveWish', draft }); setEditor(null); }} />}
  </>;
}

function WishEditor({ initial, kind, account, onClose, onSave }: { initial?: Wish; kind: WishKind; account: Account; onClose: () => void; onSave: (draft: WishDraft) => Promise<void> }) {
  ...
  return <Modal title={initial ? '编辑愿望' : '新增愿望'} wide onClose={() => { if (!submitting.current) onClose(); }}><form onSubmit={event => { event.preventDefault(); void submit(); }}>
    <fieldset disabled={saving} className="wish-editor-fields"><label>名称<input autoFocus required maxLength={200} value={draft.name} onChange={event => update('name', event.target.value)} /></label>
      <div className="form-grid"><label>分类<select value={draft.kind} onChange={event => update('kind', event.target.value as WishKind)}>{kinds.map(value => <option key={value} value={value}>{wishKinds[value]}</option>)}</select></label>
