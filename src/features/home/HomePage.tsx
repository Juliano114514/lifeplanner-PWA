...
function updatedLabel(at: number, now: number, zone: string) {
  ...
  if (days < 1) return `${Math.floor(Math.max(0, now - at) / 3600000)}小时前（${new Intl.DateTimeFormat('zh-CN', { timeZone: zone, hour: '2-digit', minute: '2-digit', hour12: false }).format(at)}）`;
