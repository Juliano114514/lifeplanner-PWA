...
**当前交付：六个导航入口。** 首页、任务、日程、愿望、日记和菜谱使用离线优先的双人共享数据；欢迎页与 GitHub 登录入口保留。Android 仍为本地应用，本仓库尚未使 Android 自动联网或同步。
...
打开 <http://127.0.0.1:5173>。未配置认证时可看到页面、六个导航入口及配置提示；不会出现虚假数据，也没有绕过认证的开发身份。
...
- 日记按日期展示带用户名标签的开心/不开心条目及每人一篇的当日日记。正文输入框只回显本人内容，重复保存更新本人条目，清空保存移除本人条目；开心/不开心条目仍可共同修改，保存保持原作者。
- 发布日记归属功能前应用 `0007_diary_owners.sql`：从历史保存记录恢复条目作者及现有正文最后内容修改者，保留原正文；无作者证据的旧条目显示“历史条目”。迁移提升聚合版本，旧设备未同步操作会按现有冲突流程处理。离线旧快照在联网前只能按最后编辑者临时兼容。
- 菜谱替代原库存模块，复用愿望的编辑、归属筛选、置顶、完成、归档与离线同步。旧库存、菜品和采购路径跳转至菜谱；旧数据及命令保留兼容离线队列。
- 发布菜谱前应用 `0008_recipes.sql`，一次性独立复制「想要吃」的全部未删除记录（包括已完成和已归档），原愿望保留。菜谱使用新 ID；后续两边分别编辑，不自动重复复制。
- 任务、愿望及菜谱的已归档条目支持长按 600ms 弹出删除确认，也可右键或按 Delete。删除标记随版本同步，用于防止旧离线操作恢复原记录。
- 新建任务默认归属本人；任务分别记录归属人、不可变创建人、最后修改人。
- 右上角头像打开个人资料浮窗。名称统一用于归属人、创建人、最后修改人、任务归属选择框和共享成员提示；头像、简介随账号同步。离线编辑先保存在本机，多设备修改以最后到达服务端的保存为准。
- 升级个人资料功能前，先应用 `0003_profiles.sql`（`npm run db:remote`），再部署新版 Worker。旧版本保存在本机的资料会自动进入待同步状态。
