import { plannerCommandSchema, applyPlannerCommand, emptyPlanner, type PlannerData } from '../shared/planner';
...
async function getPlanner(env: Env): Promise<PlannerData> {
  ...
  return row ? JSON.parse(row.data) as PlannerData : emptyPlanner();
}
export async function plannerRoute(request: Request, env: Env, actor: Identity): Promise<Response | null> {
  ...
  const replay = async (): Promise<Response | null> => {
    ...
    return json({ planner: JSON.parse(prior.result) as PlannerData });
  };
