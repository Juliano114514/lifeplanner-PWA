...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    try {
      ...
      return await eggRoute(request, env, person) ?? await profileRoute(request, env, person) ?? await tasksRoute(request, env, person) ?? await plannerRoute(request, env, person) ?? json({ error: 'NOT_FOUND', message: '接口不存在' }, 404);
    } catch (error) {
